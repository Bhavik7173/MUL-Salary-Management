...
# ── FIX 1 & 2: CORS added BEFORE router, origins from env ────
app.add_middleware(
    CORSMiddleware,
    allow_credentials=True,
...
class Settings(BaseModel):
    ...
    auto_email_day: int = 1
    dark_mode: bool = False
    manual_azk_adjustment: float = 0.0
    daily_standard_hours: float = DEFAULT_DAILY_STD_HOURS
...
def calculate_working_hours(start_time: str, end_time: str, break_hours: float) -> float:
    try:
        ...
        if end < start:
            ...
            end = end.replace(day=2); start = start.replace(day=1)
        total = (end - start).seconds / 3600
        return round(max(0, total - break_hours), 2)
    except:
        return 0.0
...
def calculate_entry_pay(entry_data: dict, settings: Settings) -> dict:
    """
    gross        = working_hours × hourly_rate   (taxable only)
    tax          = gross × tax_rate
    net_earned   = gross − tax
    final_payout = net_earned + bonus + travel + meal  (all post-tax, tax-free)
    """
    working_hours = calculate_working_hours(
        entry_data.get("start_time", "00:00"),
        entry_data.get("end_time",   "00:00"),
    ...
    travel = float(entry_data.get("travel_allowance", 0) or 0)
    meal   = float(entry_data.get("meal_allowance",   0) or 0)
    gross  = round(working_hours * settings.hourly_rate, 2)
    tax    = round(gross * settings.tax_rate, 2)
    net    = round(gross - tax, 2)
    final  = round(net + bonus + travel + meal, 2)
    return {
        "working_hours": round(working_hours, 2),
        "bonus":         round(bonus, 2),
        "gross_pay":     gross,
...
async def get_azk_bank_total(exclude_year: int = None, exclude_month: int = None) -> float:
    ...
    for r in results:
        yr, mo = r["_id"]["year"], r["_id"]["month"]
        if exclude_year and exclude_month and yr == exclude_year and mo == exclude_month:
            continue
        ph_credit = r["absent_holidays"] * settings.daily_standard_hours
        try:
            vac_list = await db.vacation_entries.find({"year": yr, "month": mo}).to_list(100)
        except Exception:
            vac_list = []
        vac_hours = sum(v.get("days", 0) for v in vac_list) * settings.daily_standard_hours
        azk_total += (r["worked_hours"] + ph_credit + vac_hours) - settings.contract_hours
    return round(azk_total + settings.manual_azk_adjustment, 2)
...
async def create_work_entry(entry: WorkEntryCreate):
    ...
    entry_dict = entry.model_dump()
    pay_data   = calculate_entry_pay(entry_dict, settings)
    work_entry = WorkEntry(**entry_dict, **pay_data)
    await db.work_entries.insert_one(work_entry.model_dump())
    return work_entry
...
async def get_work_entries(year: Optional[int] = None, month: Optional[int] = None):
    ...
    return await db.work_entries.find(query, {"_id": 0}).sort("date", -1).to_list(1000)
...
async def update_work_entry(entry_id: str, update: WorkEntryUpdate):
    ...
    if update_dict:
        entry.update(update_dict)
        settings = await get_settings()
        entry.update(calculate_entry_pay(entry, settings))
        entry["updated_at"] = datetime.now(timezone.utc).isoformat()
        await db.work_entries.update_one({"id": entry_id}, {"$set": entry})
    return WorkEntry(**entry)
...
async def get_monthly_summary(year: int, month: int):
    settings = await get_settings()
    entries  = await db.work_entries.find(
        {"date": {"$regex": f"^{year}-{month:02d}"}}, {"_id": 0}
    ).sort("date", 1).to_list(200)
    ...
    try:
        vac_list = await db.vacation_entries.find({"year": year, "month": month}).to_list(100)
    except Exception:
        vac_list = []
    vacation_days  = round(sum(v.get("days", 0) for v in vac_list), 2)
    vacation_hours = round(vacation_days * settings.daily_standard_hours, 2)

    # ── FIX 6: use try/except for sick_days instead of hasattr ──
    try:
        sick_list = await db.sick_days.find({"year": year, "month": month}).to_list(100)
    except Exception:
        sick_list = []
    sick_days = round(sum(s.get("days", 0) for s in sick_list), 2)
    ...
    gross_pay    = round(payable_hours * settings.hourly_rate, 2)
    tax          = round(gross_pay * settings.tax_rate, 2)
    net_pay      = round(gross_pay - tax, 2)
    final_payout = round(net_pay + bonus_total + travel_total + meal_total, 2)

    daily_hours = [
        {"date": e["date"], "hours": effective_hours(e),
         "is_public_holiday": e.get("is_public_holiday", False), "day": e["date"][-2:]}
        for e in entries
    ]
...
# ============= YEARLY SUMMARY =============
# FIX 7: renamed to /yearly/{year} to avoid route conflict with /summary/{year}/{month}

@api_router.get("/yearly/{year}", response_model=YearlySummary)
async def get_yearly_summary(year: int):
    settings    = await get_settings()
    months_data = []
    ...
    for r in pre_results:
        try:
            vac_l = await db.vacation_entries.find({"year": r["_id"]["year"], "month": r["_id"]["month"]}).to_list(100)
        except Exception:
            vac_l = []
        ph_h = r["absent_holidays"] * settings.daily_standard_hours
        vac_h = sum(v.get("days", 0) for v in vac_l) * settings.daily_standard_hours
    ...
    for m in range(1, 13):
        if m in raw:
            r = raw[m]
            try:
                vac_l  = await db.vacation_entries.find({"year": year, "month": m}).to_list(100)
            except Exception:
                vac_l = []
            try:
                sick_l = await db.sick_days.find({"year": year, "month": m}).to_list(100)
            except Exception:
                sick_l = []
            vac_days  = round(sum(v.get("days", 0) for v in vac_l), 2)
            sick_days = round(sum(s.get("days", 0) for s in sick_l), 2)
            ...
            azk_change = round(total_h - settings.contract_hours, 2)
            azk_running = round(azk_running + azk_change, 2)
            payable = round(settings.contract_hours + azk_running, 2) if (azk_change < 0 and azk_running < 0) else settings.contract_hours
            bonus_t = round(r["bonus_total"], 2)
            travel_t = round(r["travel_total"], 2)
            meal_t   = round(r.get("meal_total", 0), 2)
            gross    = round(payable * settings.hourly_rate, 2)
            ...
            months_data.append(MonthRow(
                month=m, month_name=MONTH_NAMES[m-1],
                total_worked_hours=round(r["worked_hours"], 2),
                payable_hours=payable, azk_change=azk_change, azk_bank_total=azk_running,
                gross_pay=gross, tax=tax, bonus_total=bonus_t, travel_total=travel_t,
                meal_total=meal_t, vacation_days=vac_days, sick_days=sick_days,
                net_pay=net, final_payout=final, has_data=True
            ))
        else:
            months_data.append(MonthRow(
                month=m, month_name=MONTH_NAMES[m-1],
                total_worked_hours=0, payable_hours=0, azk_change=0,
                azk_bank_total=round(azk_running, 2), gross_pay=0, tax=0,
                bonus_total=0, travel_total=0, meal_total=0, vacation_days=0,
                sick_days=0, net_pay=0, final_payout=0, has_data=False
            ))
...
@api_router.get("/vacation/balance", response_model=List[VacationBalance])
async def get_vacation_balances():
    balances = await db.vacation_balances.find({}).to_list(50)
    result = []
    for b in balances:
        used_list = await db.vacation_entries.aggregate([
            {"$match": {"balance_year": b["balance_year"]}},
            {"$group": {"_id": None, "total": {"$sum": "$days"}}}
        ]).to_list(1)
        used = round(used_list[0]["total"] if used_list else 0.0, 1)
        result.append(VacationBalance(
            balance_year=b["balance_year"], total_entitlement=b["total_entitlement"],
            used=used, remaining=round(b["total_entitlement"] - used, 1)
        ))
    return result
...
async def get_vacation_entries(year: Optional[int] = None, month: Optional[int] = None):
    ...
    if year:  query["year"]  = year
    if month: query["month"] = month
    return [VacationEntry(**e) for e in await db.vacation_entries.find(query).sort("date", 1).to_list(500)]
...
async def create_vacation_entry(data: VacationEntryCreate):
    try:
        ...
        raise HTTPException(status_code=400, detail="Date must be YYYY-MM-DD")
    entry = VacationEntry(date=data.date, year=parsed.year, month=parsed.month,
                          days=data.days, balance_year=data.balance_year, notes=data.notes)
    await db.vacation_entries.insert_one(entry.model_dump())
    return entry
...
async def get_sick_days(year: Optional[int] = None, month: Optional[int] = None):
    ...
    if year:  query["year"]  = year
    if month: query["month"] = month
    return [SickDay(**s) for s in await db.sick_days.find(query).sort("date", 1).to_list(500)]
...
async def create_sick_day(data: SickDayCreate):
    try:
        ...
        raise HTTPException(status_code=400, detail="Date must be YYYY-MM-DD")
    entry = SickDay(date=data.date, year=parsed.year, month=parsed.month,
                    days=data.days, notes=data.notes)
    await db.sick_days.insert_one(entry.model_dump())
    return entry
...
@api_router.get("/sick-days/summary")
async def get_sick_days_summary(year: int):
    entries  = await db.sick_days.find({"year": year}).sort("date", 1).to_list(500)
    by_month = {}
    for e in entries:
        m = e["month"]
        by_month[m] = round(by_month.get(m, 0) + e["days"], 1)
...
async def update_settings(update: SettingsUpdate):
    ...
    update_dict   = {k: v for k, v in update.model_dump().items() if v is not None}
    settings_dict.update(update_dict)
    await db.settings.update_one({"id": "settings"}, {"$set": settings_dict}, upsert=True)
    if "hourly_rate" in update_dict or "tax_rate" in update_dict:
        new_settings = Settings(**settings_dict)
        entries = await db.work_entries.find({}, {"_id": 0}).to_list(10000)
        for entry in entries:
            await db.work_entries.update_one(
                {"id": entry["id"]},
                {"$set": calculate_entry_pay(entry, new_settings)}
            )
    return Settings(**settings_dict)
...
async def recalculate_all_entries():
    ...
    for entry in entries:
        await db.work_entries.update_one(
            {"id": entry["id"]},
            {"$set": calculate_entry_pay(entry, settings)}
        )
    return {"message": f"Recalculated {len(entries)} entries with current settings"}
...
async def delete_entries_before(year: int, month: int):
    ...
    for y in range(2020, year + 1):
        for m in range(1, 13):
            if y < year or (y == year and m < month):
                r = await db.work_entries.delete_many({"date": {"$regex": f"^{y}-{m:02d}"}})
                deleted += r.deleted_count
    return {"message": f"Deleted {deleted} entries before {year}-{month:02d}"}
...
async def debug_azk_breakdown():
    ...
    for r in results:
        ...
        ph_credit = r["absent_holidays"] * settings.daily_standard_hours
        try:
            vac_l = await db.vacation_entries.find({"year": yr, "month": mo}).to_list(100)
        except Exception:
            vac_l = []
        vac_hours  = sum(v.get("days", 0) for v in vac_l) * settings.daily_standard_hours
        effective  = r["worked_hours"] + ph_credit + vac_hours
        ...
        running    = round(running + azk_change, 2)
        breakdown.append({
            "month": f"{yr}-{mo:02d}", "entries": r["entry_count"],
            "worked_hours": round(r["worked_hours"], 2),
            "ph_credit": round(ph_credit, 2), "vac_hours": round(vac_hours, 2),
            "effective": round(effective, 2), "azk_change": azk_change, "bank_after": running,
        })
    return {"manual_adjustment": settings.manual_azk_adjustment,
            "contract_hours": settings.contract_hours,
            "daily_std_hours": settings.daily_standard_hours,
            "breakdown": breakdown, "final_bank": running}
...
async def upload_file(file: UploadFile = File(...)):
    try:
        ...
        column_mapping = {
            'date':             ['date', 'Date', 'datum', 'Datum'],
            'start_time':       ['start_time', 'Start Time', 'start', 'Start', 'begin'],
            'end_time':         ['end_time',   'End Time',   'end',   'End',   'einde'],
            'break_hours':      ['break_hours', 'Break', 'break', 'Break Hours', 'pauze'],
            'travel_allowance': ['travel_allowance', 'travel_eur', 'Travel', 'travel'],
            'meal_allowance':   ['meal_allowance',   'meal_eur',   'Meal',   'meal', 'verpflegung'],
            'is_public_holiday':['is_public_holiday','public_holiday','Holiday','holiday'],
            'notes':            ['notes', 'Notes'],
        }
        mapped_cols = {}
        for target, options in column_mapping.items():
        ...
        if 'date' not in mapped_cols:
            raise HTTPException(status_code=400, detail="Could not find date column")
        ...
        for _, row in df.iterrows():
            ...
            # ── FIX 4: keep absent PH rows (they credit 7.42h to AZK) ──
            if not start_time or not end_time:
                if is_ph:
                    # Save as absent PH entry with 00:00 times
                    start_time = "00:00"
                    end_time   = "00:00"
                else:
                    skipped_rows += 1; continue
...
async def save_uploaded_entries(entries: List[WorkEntryCreate]):
    ...
    saved = skipped = 0
    for entry in entries:
        if await db.work_entries.find_one({"date": entry.date}):
            skipped += 1; continue
        entry_dict = entry.model_dump()
        work_entry = WorkEntry(**entry_dict, **calculate_entry_pay(entry_dict, settings))
        await db.work_entries.insert_one(work_entry.model_dump())
        saved += 1
    return {"message": f"Saved {saved} entries, skipped {skipped} duplicates",
...
async def generate_payslip_pdf(year: int, month: int):
    ...
    elements = []
    elements.append(Paragraph(settings.company_name,
        ps('T', fontSize=22, fontName='Helvetica-Bold', textColor=colors.HexColor('#0F172A'), spaceAfter=4)))
    elements.append(Paragraph(f"Official Payslip · {datetime(year, month, 1).strftime('%B %Y')}",
        ps('S', fontSize=13, textColor=colors.HexColor('#64748B'), spaceAfter=20)))

    meal_t = summary.meal_total or 0
    final  = summary.final_payout
    ...
    if summary.vacation_days:
        rows.append(['Vacation Days', f'{summary.vacation_days:.1f} days', ''])
    if summary.sick_days:
        rows.append(['Sick Days', f'{summary.sick_days:.1f} days', ''])
    rows += [
        ['', '', ''],
        ['Hourly Rate',         f'€{settings.hourly_rate:.4f}/hr', ''],
        ['Base Pay (taxable)',  f'{summary.payable_hours:.2f} × €{settings.hourly_rate:.4f}', f'€{summary.gross_pay:.2f}'],
        [f'Tax ({settings.tax_rate*100:.2f}%)', '', f'-€{summary.tax:.2f}'],
        ['Net Earned',          '', f'€{summary.net_pay:.2f}'],
        ['', '', ''],
        ['Bonus (tax-free)',    '', f'€{summary.bonus_total:.2f}'],
        ['Travel (tax-free)',   '', f'€{summary.travel_total:.2f}'],
        ['Meal (tax-free)',     '', f'€{meal_t:.2f}'],
        ['', '', ''],
        ['FINAL PAYOUT',        '', f'€{final:.2f}'],
    ]
    col_w = [220, 150, 90]
    ...
    elements.append(Paragraph(
        f"Generated {datetime.now().strftime('%Y-%m-%d %H:%M')} · MUL Salary Tracker",
        ps('F', fontSize=8, textColor=colors.HexColor('#94A3B8'), alignment=1)
    ))
    doc.build(elements)
    buffer.seek(0)
    return StreamingResponse(buffer, media_type="application/pdf",
                             headers={"Content-Disposition": f"attachment; filename=payslip_{year}_{month:02d}.pdf"})
...
async def generate_annual_pdf(year: int):
    ...
    elements = []
    elements.append(Paragraph(settings.company_name,
        ps('T', fontSize=20, fontName='Helvetica-Bold', spaceAfter=4)))
    elements.append(Paragraph(f'Year-End Summary · {year}',
        ps('S', fontSize=13, textColor=GRAY, spaceAfter=20)))

    header = ['Month','Worked','AZK Δ','AZK Bank','Vac','Sick','Gross €','Tax €','Payout €']
    rows   = [header]
    for m in yearly.months:
        if m.has_data:
            sgn = '+' if m.azk_change >= 0 else ''
            rows.append([m.month_name[:3], f'{m.total_worked_hours:.1f}',
                         f'{sgn}{m.azk_change:.1f}', f'{m.azk_bank_total:.1f}',
                         str(m.vacation_days) if m.vacation_days else '—',
                         str(m.sick_days)     if m.sick_days     else '—',
                         f'{m.gross_pay:.2f}', f'{m.tax:.2f}', f'{m.final_payout:.2f}'])
        else:
            rows.append([m.month_name[:3]] + ['—'] * 8)
    rows.append(['TOTAL', f'{yearly.total_worked_hours:.1f}', '—', '—', '—', '—',
                 f'{yearly.total_gross_pay:.2f}', f'{yearly.total_tax:.2f}',
                 f'{yearly.total_final_payout:.2f}'])

    t = Table(rows, colWidths=[55,50,45,55,28,28,65,55,65], repeatRows=1)
    t.setStyle(TableStyle([
        ('BACKGROUND',    (0,0),  (-1,0),  BLUE),
        ('TEXTCOLOR',     (0,0),  (-1,0),  colors.white),
        ('FONTNAME',      (0,0),  (-1,0),  'Helvetica-Bold'),
    ...
    elements.append(Paragraph(
        f'Rate: €{settings.hourly_rate:.4f}/hr · Tax: {settings.tax_rate*100:.2f}% · '
        f'Contract: {settings.contract_hours:.2f} hrs/month',
        ps('I', fontSize=9, textColor=GRAY)
    ))
    elements.append(Paragraph(
        f'Generated {datetime.now().strftime("%Y-%m-%d %H:%M")} · MUL Salary Tracker',
        ps('F', fontSize=8, textColor=colors.HexColor('#94A3B8'), alignment=1)
    ...
    buffer.seek(0)
    return StreamingResponse(buffer, media_type="application/pdf",
                             headers={"Content-Disposition": f"attachment; filename=annual_{year}.pdf"})

# ============= EXCEL EXPORT =============
# FIX 5: added meal_allowance column
...
async def export_to_excel(year: int, month: int):
    ...
    if not df.empty:
        df = df[['date','start_time','end_time','break_hours','working_hours',
                 'travel_allowance','meal_allowance','is_public_holiday',
                 'bonus','gross_pay','tax','net_pay','notes']]
        df.columns = ['Date','Start','End','Break (h)','Hours',
                      'Travel (€)','Meal (€)','Public Holiday',
                      'Bonus (€)','Gross (€)','Tax (€)','Net (€)','Notes']
    summary_df = pd.DataFrame({
        'Metric': ['Total Worked Hours','Payable Hours','AZK Change','AZK Bank',
                   'Gross Pay','Tax','Bonus','Travel','Meal','Net Pay','Final Payout'],
        'Value':  [summary.total_worked_hours, summary.payable_hours, summary.azk_change,
                   summary.azk_bank_total, summary.gross_pay, summary.tax,
                   summary.bonus_total, summary.travel_total, summary.meal_total,
                   summary.net_pay, summary.final_payout]
    })
    buffer = io.BytesIO()
    with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
        df.to_excel(writer, sheet_name='Work Entries', index=False)
...
async def send_email_with_payslip(request: EmailRequest):
    ...
    if not settings.email_address or not settings.email_password:
        raise HTTPException(status_code=400, detail="Email not configured in settings")
    summary = await get_monthly_summary(request.year, request.month)
    month_label = datetime(request.year, request.month, 1).strftime('%B %Y')
    msg = MIMEMultipart()
    msg['From']    = settings.email_address
    msg['To']      = request.recipient_email
    msg['Subject'] = f"Payslip · {month_label}"
    body = f"""<html><body style="font-family:Arial,sans-serif;max-width:520px">
<h2 style="color:#2563EB">{settings.company_name}</h2>
<p>Payslip for <strong>{month_label}</strong>.</p>
<table style="border-collapse:collapse;width:100%;margin:16px 0">
  <tr><td style="padding:8px;border:1px solid #e2e8f0">Worked Hours</td>
      <td style="padding:8px;border:1px solid #e2e8f0"><b>{summary.total_worked_hours:.2f} hrs</b></td></tr>
  <tr style="background:#f8fafc"><td style="padding:8px;border:1px solid #e2e8f0">Gross Pay</td>
      <td style="padding:8px;border:1px solid #e2e8f0">€{summary.gross_pay:.2f}</td></tr>
  <tr><td style="padding:8px;border:1px solid #e2e8f0">Tax</td>
      <td style="padding:8px;border:1px solid #e2e8f0">€{summary.tax:.2f}</td></tr>
  <tr style="background:#eff6ff"><td style="padding:8px;border:1px solid #2563EB"><b>Final Payout</b></td>
      <td style="padding:8px;border:1px solid #2563EB;color:#2563EB;font-size:16px"><b>€{summary.final_payout:.2f}</b></td></tr>
</table>
</body></html>"""
    msg.attach(MIMEText(body, 'html'))
    try:
...
async def auto_send_payslip():
    ...
    msg['To']      = settings.email_address
    msg['Subject'] = f"Payslip · {month_label}"
    msg.attach(MIMEText(f"<p>Payslip {month_label}: Final Payout €{summary.final_payout:.2f}</p>", 'html'))
    try:
        srv = smtplib.SMTP(settings.smtp_server, settings.smtp_port)
        srv.starttls()
        srv.login(settings.email_address, settings.email_password)
        srv.send_message(msg)
        srv.quit()
        return {"message": f"Payslip for {month_label} sent", "sent": True}
    except Exception as e:
